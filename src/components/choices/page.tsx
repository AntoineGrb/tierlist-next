...
const Choices = ({items, isMobile}: ChoicesProps) => {
    ...
    return (
        ...
        <Droppable droppableId='choices' direction='horizontal'>
            {(provided) => (
                <div ref={provided.innerRef} {...provided.droppableProps} className={`fixed bottom-0 left-0 w-full min-h-20 flex flex-nowrap overflow-x-auto bg-gray-200 shadow-md justify-start items-center gap-1 p-2 pt-8 rounded-t-md sm:rounded-lg sm:relative sm:mt-1 sm:overflow-x-auto sm:flex-wrap`}>
